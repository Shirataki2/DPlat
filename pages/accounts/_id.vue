...
class Index extends Vue {
  ...
  async mounted () {
    ...
    const [snapUser, snapStat, snapRooms, snapCompetitions] = await Promise.all([
      ...
      db.collection('status').doc(this.$route.params.id).get(),
      db.collection('users').doc(this.$route.params.id).collection('chatrooms').get(),
      db.collection('users').doc(this.$route.params.id).collection('competitions').get()
    ])
    this.isLoginUser = this.$route.params.id === authStore.user.uid
    if (snapUser.exists) {
